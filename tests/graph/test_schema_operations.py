...
async def test_schema_ops_get_schema_structure_valid(adapter: BaseGraphAdapter):
    ...
    assert any(key in schema for key in ['nodes', 'edges', 'vertexLabels', 'edgeLabels', 'types']), \
...
async def test_schema_ops_schema_cached_in_standalone_mode(adapter: BaseGraphAdapter):
    ...
    ctx = make_ctx(GraphContext, request_id="t_schema_cache", tenant="t-cache", metrics=metrics)
...
async def test_schema_ops_schema_consistency(adapter: BaseGraphAdapter):
    ...
    assert schema1.keys() == schema2.keys(), "Schema structure should be consistent across calls"
