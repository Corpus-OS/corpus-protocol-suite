...
if TYPE_CHECKING:
    # Type-checking-only imports; require SK in dev environment.
...
else:  # pragma: no cover - runtime import surface
    try:
        from semantic_kernel.connectors.ai.chat_completion_client_base import (
            ChatCompletionClientBase,
            ChatHistory,
...
def _extract_text_from_chat_message(msg: Any) -> str:
    ...
    - As a last resort, log a warning and return an empty string.
    """
    ...
    # Final fallback: avoid repr-like noise; log and return empty.
    logger.warning(
        "Failed to extract text from Semantic Kernel chat message; "
        "returning empty string. Message repr: %r",
        msg,
    )
    return ""
