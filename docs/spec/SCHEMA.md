...
5. **Where extensibility is allowed (request envelope + ctx + args pass-through)**
6. **Where it is prohibited (success/error envelopes, type defs unless stated)**
7. **Why (forward-compat + vendor extensions at the boundary; strictness for outputs/contracts)**
    ...
    { "$ref": "https://corpusos.com/schemas/embedding/embedding.envelope.request.json" },
    {
      "properties": {
        ...
        "args": {
          "type": "object",
          "additionalProperties": true,
          "description": "Args object is required on the wire; may be empty; extra keys are ignored."
        }
      }
    }
  ]
...
* `text` non-empty
* `model` non-empty
* `stream` must be **absent or false** (streaming uses separate op)
