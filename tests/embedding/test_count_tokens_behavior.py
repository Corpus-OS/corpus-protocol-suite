...
async def test_token_counting_monotonic_with_text_length(adapter: BaseEmbeddingAdapter):
    ...
    for i in range(1, len(counts)):
        ...
        ), f"Non-monotonic: {counts[i-1]} -> {counts[i]} for texts: '{texts[i-1]}' -> '{texts[i]}'"
...
async def test_token_counting_unknown_model_raises_model_not_available(
...
async def test_token_counting_invalid_input_raises_bad_request(
    adapter: BaseEmbeddingAdapter,
):
