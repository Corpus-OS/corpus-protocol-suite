...
{
  "ok": false,
  "error": "ResourceExhausted",          // canonical class or subtype
  "message": "Rate limit exceeded for tenant",
  "code": "RATE_LIMIT",                  // adapter-specific short string
  "http_status": 429,                    // transport-mapped (if HTTP)
  "retry_after_ms": 1200,                // non-negative int or null
  "resource_scope": "rate_limit",        // finite bounded set
  "throttle_scope": "tenant:acme:llm",   // optional, bounded string
  "suggested_batch_reduction": 50,       // percent [0..100]
  "details": {
    "max_batch_size": 1000,
...
2.1 Normative Rules
	...
	•	code is an adapter-specific short identifier for logs and dashboards.
It is not the same as the metrics code label (see §5).
...
{"max_context_length": <int>, "provided_tokens": <int>, "model": "<name>"}
	...
	•	details may include:
{"policy_section": "safety.v2", "category": "HATE"}
	•	InputFormatError
...
Adapters SHOULD attach structured hints:
	...
	•	resource_scope — one of:
"model" | "token_limit" | "rate_limit" | "memory" | "compute" | "time_budget" | "index" | "shard"
