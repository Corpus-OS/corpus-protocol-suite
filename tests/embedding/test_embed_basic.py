...
async def test_core_ops_embed_returns_valid_embedding_structure(
    ...
    assert all(
        isinstance(v, (int, float)) for v in res.embedding.vector
    ), "Vector must contain numbers"

    # Validate dimensions match vector length
    assert (
        res.embedding.dimensions == len(res.embedding.vector)
    ...
    if res.truncated is not None:
        assert isinstance(
            res.truncated, bool
        ), "truncated must be boolean if present"
...
async def test_core_ops_embed_requires_valid_text(adapter: BaseEmbeddingAdapter):
    ...
    for text, description in invalid_cases:
        ...
        assert any(
            term in error_msg for term in ["text", "input", "invalid", "empty"]
...
async def test_core_ops_embed_unknown_model_clear_error(adapter: BaseEmbeddingAdapter):
    ...
    error_msg = str(exc_info.value).lower()
    assert any(
        term in error_msg for term in ["model", "available", "support", "unknown"]
    ), f"Error should mention model issue: {error_msg}"


async def test_core_ops_embed_truncation_behavior_matches_capabilities(
    adapter: BaseEmbeddingAdapter,
):
...
async def test_core_ops_embed_normalization_produces_unit_vectors(
    adapter: BaseEmbeddingAdapter,
):
    ...
    for text in test_texts:
        ...
        assert (
            0.99 <= norm <= 1.01
        ), f"Normalized vector should have unit length, got {norm} for text: '{text}'"


async def test_core_ops_embed_normalization_unsupported_raises_clear_error(
    adapter: BaseEmbeddingAdapter,
):
...
async def test_core_ops_embed_vector_quality_and_consistency(
    adapter: BaseEmbeddingAdapter,
):
    ...
    assert (
        result1.embedding.vector == result2.embedding.vector
    ...
    assert (
        result1.embedding.vector != result3.embedding.vector
    ), "Different inputs should produce different embeddings"


async def test_core_ops_embed_special_character_handling(
    adapter: BaseEmbeddingAdapter,
):
...
async def test_core_ops_embed_dimensions_consistent_with_capabilities(
    adapter: BaseEmbeddingAdapter,
):
