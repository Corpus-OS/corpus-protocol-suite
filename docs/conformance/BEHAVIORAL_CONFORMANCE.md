...
> **Tags** highlight intent; **Flake risk** is a heuristic for CI triage.

### 5.1 Embedding — `tests/embedding/`

| File                                 | Purpose / Tags                                    | Flake risk |
| ------------------------------------ | ------------------------------------------------- | ---------- |
| `run_conformance.py`                 | harness / ordering / environment smoke            | low        |
| `test_embed_basic.py`                | **core unary**; required fields; model support    | low        |
| `test_embed_batch_basic.py`          | **batch semantics**; per-index failures; ordering | low        |
| `test_truncation_and_text_length.py` | **truncate** flags; `max_text_length`             | low        |
| `test_normalization_semantics.py`    | **normalize**; `normalizes_at_source`             | low        |
| `test_count_tokens_behavior.py`      | **token monotonicity**; unicode; model gating     | low        |
| `test_cache_and_batch_fallback.py`   | **caching/idempotency**; batch→per-item fallback  | medium     |
| `test_error_mapping_retryable.py`    | **taxonomy mapping**; **retry hints**             | low        |
| `test_deadline_enforcement.py`       | **budgets**; fast-fail; propagation               | medium     |
| `test_capabilities_shape.py`         | **caps truthfulness** vs behavior                 | low        |
| `test_health_report.py`              | **degraded health** shape + meaning               | low        |
| `test_context_siem.py`               | **observability/SIEM**; one-observe; no PII       | low        |
| `test_wire_handler.py`               | **canonical envelopes**; unknown ops              | low        |

### 5.2 Graph — `tests/graph/`

| File                              | Purpose / Tags                                       | Flake risk |
| --------------------------------- | ---------------------------------------------------- | ---------- |
| `test_crud_basic.py`              | vertex/edge **CRUD lifecycle**                       | low        |
| `test_query_basic.py`             | **query lifecycle**; result coherence                | low        |
| `test_streaming_semantics.py`     | **frames**; **one terminal**; no data after terminal | medium     |
| `test_batch_operations.py`        | **batch + partial success**; ack semantics           | medium     |
| `test_schema_operations.py`       | DDL/dialect **schema ops behavior**                  | low        |
| `test_dialect_validation.py`      | dialect flags & validation                           | low        |
| `test_capabilities_shape.py`      | **caps truthfulness**                                | low        |
| `test_error_mapping_retryable.py` | **taxonomy mapping**                                 | low        |
| `test_deadline_enforcement.py`    | **budgets**                                          | medium     |
| `test_health_report.py`           | health under degradation                             | low        |
...
| File                                 | Purpose / Tags                           | Flake risk |
| ------------------------------------ | ---------------------------------------- | ---------- |
| `test_complete_basic.py`             | **completion lifecycle**; params honored | low        |
| `test_streaming_semantics.py`        | **data/end/error** stream rules          | medium     |
| `test_count_tokens_consistency.py`   | **token monotonicity/consistency**       | low        |
| `test_sampling_params_validation.py` | **sampling semantics**; validation       | low        |
| `test_message_validation.py`         | **message/tool-calling surfaces**        | low        |
| `test_capabilities_shape.py`         | caps truthfulness                        | low        |
| `test_error_mapping_retryable.py`    | taxonomy mapping                         | low        |
| `test_deadline_enforcement.py`       | budgets                                  | medium     |
| `test_health_report.py`              | degraded health                          | low        |
| `test_context_siem.py`               | observability/SIEM                       | low        |
| `test_wire_handler.py`               | canonical envelopes                      | low        |
...
| File                              | Purpose / Tags                     | Flake risk |
| --------------------------------- | ---------------------------------- | ---------- |
| `test_upsert_basic.py`            | **upsert lifecycle**               | low        |
| `test_query_basic.py`             | **query lifecycle**; scoring shape | low        |
| `test_dimension_validation.py`    | **dimension caps**; consistency    | low        |
| `test_filtering_semantics.py`     | **filter correctness**             | low        |
| `test_delete_operations.py`       | **delete lifecycle**               | low        |
| `test_namespace_operations.py`    | **namespace create/delete**        | low        |
| `test_batch_size_limits.py`       | **batch limits**                   | low        |
| `test_capabilities_shape.py`      | caps truthfulness                  | low        |
| `test_error_mapping_retryable.py` | taxonomy mapping                   | low        |
| `test_deadline_enforcement.py`    | budgets                            | medium     |
| `test_health_report.py`           | health reporting                   | low        |
| `test_context_siem.py`            | observability/SIEM                 | low        |
| `test_wire_handler.py`            | canonical envelopes                | low        |
