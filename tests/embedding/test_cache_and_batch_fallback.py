...
async def test_embed_cache_behavior_observable(adapter: BaseEmbeddingAdapter):
    ...
    """
    ctx = make_ctx(OperationContext, request_id="cache_observable", tenant="t1")
    spec = EmbedSpec(
        text="cache performance test text",
    ...
    if has_caching_capability(adapter):
        assert (
            duration_2 <= duration_1 * 0.8
        ), f"Expected cached call to be faster. First: {duration_1:.3f}s, Second: {duration_2:.3f}s"
...
async def test_embed_cache_tenant_isolation_observable(adapter: BaseEmbeddingAdapter):
    ...
    # Call with tenant 1
    ctx1 = make_ctx(OperationContext, request_id="cache_t1", tenant="tenant-1")
    result_1 = await adapter.embed(spec, ctx=ctx1)

    # Call with tenant 2 - should not be cached from tenant 1
    ctx2 = make_ctx(OperationContext, request_id="cache_t2", tenant="tenant-2")
...
async def test_embed_batch_partial_failure_contract(adapter: BaseEmbeddingAdapter):
    ...
    """
    ctx = make_ctx(OperationContext, request_id="batch_partial", tenant="t")
    ...
    assert success_indices.isdisjoint(
        failure_indices
    ), "Index cannot be both successful and failed"

    # Total processed items should match input
    total_processed = len(result.embeddings) + len(result.failed_texts)
    assert (
        total_processed == len(texts)
    ), f"Expected {len(texts)} processed items, got {total_processed}"
...
async def test_embed_batch_empty_text_handling(adapter: BaseEmbeddingAdapter):
    ...
    ctx = make_ctx(OperationContext, request_id="batch_empty", tenant="t")
    ...
    empty_success_indices = {
        e.index for e in result.embeddings if e.index in empty_text_indices
    }
    empty_failure_indices = {
        f["index"] for f in result.failed_texts if f["index"] in empty_text_indices
    }
    ...
    for failure in result.failed_texts:
        if failure["index"] in empty_text_indices:
            assert "empty" in failure["error"]["message"].lower() or "invalid" in failure[
                "error"
            ]["message"].lower(), (
...
async def test_embed_batch_ordering_preserved(adapter: BaseEmbeddingAdapter):
    ...
    ctx = make_ctx(OperationContext, request_id="batch_order", tenant="t")
...
async def test_embed_error_messages_spec_compliant(adapter: BaseEmbeddingAdapter):
    ...
    """
    ctx = make_ctx(OperationContext, request_id="error_messages", tenant="t")
    ...
    try:
        ...
        assert (
            len(result.embedding.vector) > 0
        ), "Empty text should either fail or produce valid embedding"
    except Exception as e:
        # Validate error structure per spec
        error_msg = str(e).lower()
        assert any(
            term in error_msg for term in ["empty", "invalid", "text", "input"]
...
async def test_embed_cache_invalidation_observable(adapter: BaseEmbeddingAdapter):
    ...
    """
    ctx = make_ctx(OperationContext, request_id="cache_invalidation", tenant="t1")
