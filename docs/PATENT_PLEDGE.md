...
1. **Covered Activities**
   - Implementing the Corpus Protocol specifications as described in `docs/spec/`
   - Building adapters, clients, servers, or tools that comply with the protocol
   - Using, modifying, or distributing software under the Apache 2.0 license
   
   **1.5 Essential Patent Claims**
...
3. **Scope & Limitations**
   - This pledge applies only to patents owned or controlled by Interoperable Intelligence Inc.
   - It does not apply to trademarks, service marks, or certification marks
   - It does not grant rights beyond those in the Apache 2.0 license for the Software
   
   **3.5 Patent Coverage**
   - This pledge covers all patents and patent applications owned by Interoperable Intelligence Inc. as of the effective date
   - For patents acquired after this date, coverage begins 30 days after acquisition
   - "Controlled" means patents where we have the right to grant non-assertion covenants without third-party consent
   
   **3.6 Geographic Scope**
   - This pledge applies to all patents owned by Interoperable Intelligence Inc. in all jurisdictions
   - Where local law requires specific formalities, parties may request jurisdiction-specific covenants
   
   **3.7 Termination and Grandfathering**
   - Interoperable Intelligence Inc. may terminate this pledge with 90 days' written notice via public announcement at corpus.io/patents
   - Termination does not affect implementations created or deployed before the termination effective date
   - Entities in good standing at termination retain pledge benefits for their existing implementations indefinitely
   - "Written notice" means public posting plus email notification to any parties who have requested signed covenants
...
While our trademarks remain protected, any implementation that passes our conformance tests (Platinum/Gold/Silver) may state:
- ✅ "Implements Corpus Protocol v1 specification"
- ✅ "Corpus Protocol compatible"

But may not claim without authorization:
- ⚠️ "Official Corpus implementation"
- ⚠️ "Certified by Corpus" (requires certification agreement)
- ⚠️ Use of Corpus logos or wordmarks in product names

### Why We Make This Pledge
We believe that:
- **Protocols should be open** and free from patent fear
- **Innovation thrives** when developers can build without legal uncertainty
- **The ecosystem grows** when patents protect, not threaten, the community
- **Interoperability requires trust** – we're building bridges, not walls

Our patent portfolio exists to protect the ecosystem from patent trolls and bad actors, not to threaten implementers. This pledge ensures our patents serve their intended purpose: enabling innovation, not restricting it.

### Legal Effect
This pledge is a commitment by **Interoperable Intelligence Inc.** effective as of the date of publication. It supplements but does not replace the Apache 2.0 license or any Commercial License. For legal certainty, parties may request a signed patent non-assertion covenant from the Company.

This pledge applies only to Essential Patent Claims as defined above. All other patent rights are expressly reserved.

---

> **Example Scenarios:**
>
> ✅ **Protected:** You build an open-source adapter for a new vector database that implements `vector/v1` specification.
>
> ✅ **Protected:** Your company creates an internal Corpus-compatible routing service for employee use.
>
> ✅ **Protected:** A startup builds a commercial API gateway using the Corpus Protocol specifications.
>
> ✅ **Protected:** Company X and Company Y both use Corpus; X sues Y over unrelated payment processing patents.
>
...
>
> ⚠️ **Separate License Needed:** You want to sell a "Certified Corpus Router" using our trademarks and certification marks.
